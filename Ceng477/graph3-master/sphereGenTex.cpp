...
int nrChannels = 0;   
vec3 camera_dir = vec3(0.0f, 1.0f, 0.0f);
vec3 camera_gaze = vec3(0.0f, -1.0f, 0.0f);
vec3 camera_v = vec3(0.0f, 0.0f, 1.0f);
...
void set_uniforms(mat4 &mod_proj_view, mat4 &MV, mat4 &norm_trans){

    glUniformMatrix4fv(glGetUniformLocation(ID, "mod_proj_view"), 1, GL_FALSE, &mod_proj_view[0][0]);
    glUniformMatrix4fv(glGetUniformLocation(ID, "MV"), 1, GL_FALSE, &MV[0][0]);
    ...
    glUniform3fv(glGetUniformLocation(ID, "center"),1,&center[0]);
...
void readTexture(char* filename, int f ){
...
void initialize_shader(){
    ...
    if (!readDataFromFile("sphereshader.vert", v_code)) {
        cout << "Cannot find file name: shader.vert" << endl;
        exit(-1);
    }

    GLint lengthvert = v_code.length();
    const GLchar* shaderVert = (const GLchar*) v_code.c_str();
     if (!readDataFromFile("sphereshader.frag", f_code)) {
        cout << "Cannot find file name: hader.frag" << endl;
        exit(-1);
    }

    GLint lengthfrag = v_code.length();
    const GLchar* shaderFrag = (const GLchar*) f_code.c_str();

    unsigned int vertex = 0;
    unsigned int fragment = 0;
    vertex = glCreateShader(GL_VERTEX_SHADER);
    glShaderSource(vertex, 1, &shaderVert, &lengthvert);
    glCompileShader(vertex);

        // fragment Shader
    fragment = glCreateShader(GL_FRAGMENT_SHADER);   
    glShaderSource(fragment, 1, &shaderFrag, &lengthfrag);
...
mat4 camera_init(){
    ...
    camera_pos.z = 0; 
    mat4 view = mat4(1.0f);
    vec3 camera_target = camera_pos + camera_gaze*0.1f ; //*0.1f
...
vec3 computeVertice(int verStep,int horStep){
    float PI = pi<float>();
    float alpha = PI*2*float(horStep)/float(numOfSector);
    float beta = PI*float(verStep)/float(numOfStack);
...
void vertice_conf(){
    ...
    v_size = (numOfStack+1)*(numOfSector+1);
    vertexCol = new float[v_size*5];
    //BURAYA TRIANGLELAR GELICEK
    int k = 0;
    int l = 0;
    for(int i=0; i<numOfStack+1; i++){
        for(int j=0; j<numOfSector+1; j++){
            vec3 ver = computeVertice(i,j);
            vertexCol[k] = ver.x;
            vertexCol[k+1] = ver.y;
            vertexCol[k+2] = ver.z;
            vertexCol[k+3] = float(j);
            vertexCol[k+4] = float(i);
            k = k+5;
    ...
    int i_size = (numOfSector)*(numOfStack)*2;                // checked
    indices = new unsigned int[i_size*3];              
    k2 = 0;

    for(int i=0; i<numOfStack; i++){
        unsigned int currStack = i * (numOfSector+1);
        unsigned int newStack = currStack+numOfSector+1;
        for(int j=0; j<numOfSector; j++){
                indices[k2] = currStack;
                indices[k2+1] = newStack;
                indices[k2+2] = currStack+1;
                k2+=3;
                indices[k2]=currStack+1;
                indices[k2+1]=newStack;
                indices[k2+2]=newStack+1;
                k2+=3;
            currStack++;
            newStack++;
        }
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float)*k, vertexCol, GL_STATIC_DRAW);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int)*k2, indices, GL_STATIC_DRAW);
...
void processInput(GLFWwindow *window ,int key, int scode, int act, int mods)
{
    ...
    if(glfwGetKey(window, GLFW_KEY_I) == GLFW_PRESS){
        ...
        camera_speed = 0.0f;
        camera_dir = vec3(0.0f, 1.0f, 0.0f);
        camera_gaze = vec3(0.0f, -1.0f, 0.0f);
        camera_v = vec3(0.0f, 0.0f, 1.0f);
        camera_u = vec3(0.0f, 0.0f, 0.0f);
        camera_u = normalize(cross(camera_v,camera_gaze));        
        camera_pos.x = 0;
        camera_pos.y = 600;
        camera_pos.z = 0;
        ...
        offset = 0.0f;       
        heightFactor = 0.0f;
        set_uniforms(mod_proj_view,MV,norm_trans);     
    }   
    ...
    if(glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS){
        // hf inc
        offset = offset + 1.0f/float(numOfSector);
        glUniform1f(glGetUniformLocation(ID, "offset"), offset);
    }       
...
int main(int argc, char *argv[]){
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glBindVertexArray(VAO);
        camera_comp();
        glDrawElements(GL_TRIANGLES, k2 , GL_UNSIGNED_INT, 0);          
